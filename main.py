...
try:
    ...
    if connection:
        ...
        reports_ver_queries = [
            ...
            "CREATE TABLE if not exists reports_versions_table (report_uid uuid, version_number smallint NOT NULL, data json NOT NULL, updated_at timestamp without time zone NOT NULL);",

            "CREATE TABLE if not exists reports_table (report_uid uuid NOT NULL PRIMARY KEY, version_number smallint, date timestamp without time zone NOT NULL, shift text NOT NULL, end_ring smallint NOT NULL, end_chainage real NOT NULL, reported_by text NOT NULL, status text NOT NULL);",
        ...
        cycle_time_queries = [
            "CREATE TABLE if not exists cycle_time_table (report_uid uuid, event varchar(10) NOT NULL, downtime_id smallint, breakdown_id smallint, manufacture_defect boolean, remarks text, start_time timestamp without time zone NOT NULL, end_time timestamp without time zone NOT NULL, ring_number smallint NOT NULL, status_id smallint);",
            ...
            "CREATE TABLE if not exists cycle_time_table (manufacture_defect varchar(50), remarks text, start_time timestamp without time zone, end_time timestamp without time zone, ring_number smallint);",

            "INSERT INTO cycle_time_table (manufacture_defect, remarks, start_time, end_time, ring_number) VALUES (false, 'mauris vulputate elementum nullam varius', '2023-10-25 10:40:19', '2023-01-28 21:25:16', 33), (true, 'pulvinar sed nisl nunc rhoncus', '2023-01-05 22:21:41', '2023-09-07 05:32:02', 118), (true, 'volutpat sapien arcu sed augue', '2023-02-17 08:20:13', '2022-12-28 18:03:08', 44), (false, 'imperdiet nullam orci pede venenatis', '2023-04-23 23:18:33', '2023-05-10 01:29:57', 104), (false, 'fringilla rhoncus mauris enim leo', '2023-08-31 21:25:37', '2023-04-30 08:55:10', 125);",
        ...
        # Execute each query
        for query in reports_ver_queries:
            cursor.execute(query)
        print("Reports and reports version done.")
        
        for query in other_table_queries:
            cursor.execute(query)
        print("Status, breakdown and downtime done.")

        for query in cycle_time_queries:
            cursor.execute(query)
        print("Cycle time done.")
